...
def create_order_pdf(connection: sqlite3.Connection, order_id: int, *, document_type: str, paper_format: str, destination: Path) -> Path:
    ...
    if document_type == "Fatura" and TEMPLATE.exists():
        return _fill_invoice_template(order, lines, extras, company, kdv, tax, total, destination)
...
def _fill_invoice_template(order, lines, extras, company, kdv, tax, total, destination: Path) -> Path:
